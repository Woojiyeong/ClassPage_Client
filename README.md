...
이 저장소는 **프론트만** Vercel에 올립니다. **API(NestJS)와 MySQL**은 Vercel 밖(Railway, Render, Fly.io, 자체 VPS 등)에 띄운 뒤, 아래 환경변수로 연결합니다.
...
- DB(MySQL)는 배포 환경에서 접속 가능해야 합니다.

### 2) Vercel 프로젝트 설정

1. [Vercel](https://vercel.com)에서 GitHub 저장소 `ClassPage_Client`를 Import
